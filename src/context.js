...
const DEFAULT_CONTEXT_MARKER = 'nado-agent-context';

function workerLine(worker) {
  const caps = worker.capabilities?.length ? worker.capabilities.join(', ') : 'none';
  const labels = Object.entries(worker.labels || {}).map(([key, value]) => `${key}=${value}`).join(', ') || 'none';
  const status = worker.gatewayState || (worker.active ? 'active' : 'stale');
  const slots = `${worker.runningSlots ?? worker.runningTasks ?? 0}/${worker.maxConcurrency || 1}`;
  const agentReadiness = worker.readiness?.agent || workerReadinessDiagnostics(worker).agent;
  const agent = `agent: ${agentReadiness.status}/${agentReadiness.mode}${agentReadiness.realTerminalAgent ? ' (real terminal agent)' : ''}`;
  const agentSelfTest = worker.diagnostics?.agentSelfTest
    ? `agent self-test: ${worker.diagnostics.agentSelfTest.status}${worker.diagnostics.agentSelfTest.at ? ` at ${worker.diagnostics.agentSelfTest.at}` : ''}`
    : 'agent self-test: not recorded';
  const tools = Object.entries(worker.inventory?.tools || {})
    .filter(([, tool]) => tool.available)
    .map(([name, tool]) => `${name}${tool.version ? ` (${tool.version})` : ''}`)
    .join(', ') || 'none';
  const gpu = workerResourceDiagnostics(worker).gpu;
  const gpuText = gpu.warning ? `gpu: ${gpu.source} (${gpu.warning})` : `gpu: ${gpu.source}`;
  const agentWarning = agentReadiness.warning ? `; agent warning: ${agentReadiness.warning}` : '';
  return `- ${worker.id}: ${status}; admin: ${worker.adminState || 'enabled'}; slots: ${slots}; current task: ${worker.currentTaskId || 'none'}; capabilities: ${caps}; labels: ${labels}; tools: ${tools}; ${gpuText}; ${agent}${agentWarning}; ${agentSelfTest}; last seen: ${worker.lastSeenAt || 'never'}`;
}

function mcpToolText() {
  return MCP_TOOL_NAMES.map((name) => `- \`${name}\``).join('\n');
}

function routingPolicyText() {
  const inference = CAPABILITY_INFERENCE_POLICY.map((policy) => (
    `- ${policy.capability}: inferred from ${policy.sources.join(', ')} using rules: ${policy.rules.join('; ')}`
  ));
  inference.push(`- agent readiness: agent tasks require a configured agent command; recent successful agent self-tests receive a +${AGENT_READINESS_POLICY.successfulRecentSelfTestBonus} routing score bonus for ${Math.round(AGENT_READINESS_POLICY.recentSelfTestWindowMs / 3600000)} hours; stale successes receive +${AGENT_READINESS_POLICY.successfulStaleSelfTestBonus}; failed agent self-tests receive -${AGENT_READINESS_POLICY.failedSelfTestPenalty}.`);
  if (AGENT_READINESS_POLICY.preferRealTerminalAgent) {
    inference.push(`- agent preference: agent tasks add ${AGENT_READINESS_POLICY.realTerminalAgentBonus} points for real Codex/Claude terminal agents, subtract ${AGENT_READINESS_POLICY.demoEchoAgentPenalty} points from demo echo agents, and reject Codex/Claude preset workers when the matching CLI is missing from inventory.`);
  }
  if (RESOURCE_PREFERENCE_POLICY.preserveGpuCapacityWhenGpuNotRequired) {
    inference.push(`- resource preference: non-GPU tasks subtract ${RESOURCE_PREFERENCE_POLICY.preserveGpuCapacityPenalty} points from GPU-capable workers to keep accelerators free unless GPU is required.`);
  }
  if (RESOURCE_PREFERENCE_POLICY.preferProbeDetectedGpuWhenGpuRequired) {
    inference.push(`- GPU preference: GPU-required tasks add ${RESOURCE_PREFERENCE_POLICY.gpuProbeDetectedBonus} points for workers with NVIDIA/ROCm probe evidence and subtract ${RESOURCE_PREFERENCE_POLICY.gpuAdvertisedOnlyPenalty} points from advertised-only GPU workers; advertised-only workers remain eligible as a fallback.`);
  }
  return inference.join('\n');
}

export function buildAgentContext({ controlUrl, workers, sessions = [] }) {
  const workerText = workers.length
    ? workers.map(workerLine).join('\n')
    : '- No workers registered yet.';
  const sessionText = sessions.length
    ? sessions.map((session) => `- ${session.id}: ${session.status}; worker: ${session.assignedWorkerId || session.requestedWorkerId || 'unassigned'}; current task: ${session.currentTaskId || 'none'}; title: ${session.title}`).join('\n')
    : '- No sessions yet.';

  return `# Nado Agent Control Context
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js capabilities --control ${controlUrl}
\`\`\`
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js demo reset --control ${controlUrl} --dry-run
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js demo reset --control ${controlUrl} --yes
\`\`\`
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js workers --control ${controlUrl}
\`\`\`
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker bootstrap-bundle --control ${controlUrl} --capability code --agent codex --out ./nado-worker-bootstrap.zip
\`\`\`

Generate a worker invite script for another Ubuntu/WSL host when a fixed worker ID is required:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker invite --control ${controlUrl} --id <worker-id> --capability code --agent codex --issue-token
\`\`\`
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker bundle --control ${controlUrl} --id <worker-id> --capability code --agent codex --issue-token --out ./nado-worker.zip
\`\`\`

List or revoke issued worker tokens:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker tokens --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker token revoke <token-id> --control ${controlUrl}
\`\`\`

List or revoke self-service worker enrollment tokens:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker enrollments --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker enrollments prune --dry-run --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker enrollments prune --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker enrollment revoke <enrollment-token-id> --control ${controlUrl}
\`\`\`

Submit to a specific worker:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --worker <worker-id> --title "short task title" --command "echo hello"
\`\`\`

Create a long-lived session:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js session create --control ${controlUrl} --title "subproject name" --capability code
\`\`\`

Submit into a session:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --session <session-id> --title "next step" --command "your shell command"
\`\`\`

Download a session snapshot:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js session download <session-id> --control ${controlUrl} --out ./session-output
\`\`\`

Attach a local input file:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --worker <worker-id> --file ./brief.md --title "process brief" --command "cat brief.md"
\`\`\`

Attach a local directory:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --session <session-id> --dir ./small-project --title "work on project" --command "ls -R"
\`\`\`

Download all task artifacts:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js artifacts download <task-id> --control ${controlUrl} --out ./downloads
\`\`\`

Recover a task:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js task manage <task-id> --control ${controlUrl} --action cancel
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js task manage <task-id> --control ${controlUrl} --action requeue --worker <worker-id>
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js task manage <task-id> --control ${controlUrl} --action reschedule
\`\`\`

Recover tasks stranded on offline workers:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js recover --control ${controlUrl} --action list
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js recover --control ${controlUrl} --action requeue --worker <offline-worker-id> --target-worker <worker-id>
\`\`\`

Submit by capability:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --capability code --title "implement subtask" --command "your shell command"
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --capability code --title "implement subtask" --command "your shell command" --wait --download --out ./task-output
\`\`\`

Submit by worker label:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --required-label zone=lab --priority 10 --title "labelled host task" --command "your shell command"
\`\`\`
...
\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js planner plan --control ${controlUrl} --prompt "large task to split across workers" --mode map_reduce --json
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js planner run --control ${controlUrl} --prompt "large task to split across workers" --mode map_reduce --require-routable --wait --report
...
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch submit --control ${controlUrl} --file ./batch.json
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js dispatch plan --control ${controlUrl} --file ./batch.json
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch events <batch-id> --control ${controlUrl} --watch
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch wait <batch-id> --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch report <batch-id> --control ${controlUrl}
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch download <batch-id> --control ${controlUrl} --out ./batch-output
...
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch manage <batch-id> --control ${controlUrl} --action retry_failed
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js batch manage <batch-id> --control ${controlUrl} --action cancel
\`\`\`

Submit to a worker's terminal LLM agent:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js submit --control ${controlUrl} --worker <worker-id> --type agent --title "agent subtask" --prompt "Describe the work here."
\`\`\`

Inspect a task:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js task <task-id> --control ${controlUrl}
\`\`\`

Wait for a task:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js wait <task-id> --watch --control ${controlUrl}
\`\`\`

Inspect a scheduler decision:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js schedule <task-id> --control ${controlUrl}
\`\`\`

Manage a worker:

\`\`\`bash
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action pause
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action resume
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action drain
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action shutdown
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action cancel_current
NADO_TOKEN="$NADO_TOKEN" node ./src/cli.js worker manage --control ${controlUrl} --id <worker-id> --action forget
\`\`\`
...
- When creating invites or bundles from the Dashboard, HTTP API, or CLI without an explicit bundle Control URL, Nado prefers \`NADO_PUBLIC_CONTROL_URL\` or the best non-loopback LAN/IPv6 candidate from \`GET /api/network\`; inspect \`nado network --control ${controlUrl}\`, MCP \`nado_network\`, or \`network.nextAction\`, and configure \`NADO_PUBLIC_CONTROL_URL\` or pass CLI \`--bundle-control-url\` first when the control server runs in Docker and only container bridge addresses are visible.
...
- Use \`worker preflight --control ${controlUrl} --id <worker-id>\` on a remote host before \`worker start\` when you need to verify Node.js, repo path, data directory, control reachability, and token binding.
...
export async function writeAgentContext(file, context) {
  await ensureDir(path.dirname(file));
  await fs.writeFile(file, context, 'utf8');
}

export function buildInstalledAgentContextBlock(context, marker = DEFAULT_CONTEXT_MARKER) {
  return [
    `<!-- ${marker}:start -->`,
    context.trimEnd(),
    `<!-- ${marker}:end -->`,
...
function isStandaloneNadoAgentContext(text) {
  const trimmed = String(text || '').trimStart();
  return trimmed.startsWith('# Nado Agent Control Context');
}

export async function installAgentContext(file, context, options = {}) {
  const marker = options.marker || DEFAULT_CONTEXT_MARKER;
  const target = path.resolve(file);
  const start = `<!-- ${marker}:start -->`;
  const end = `<!-- ${marker}:end -->`;
  const block = buildInstalledAgentContextBlock(context, marker);
  let existing = '';
  let mode = 'created';
  ...
  if (startIndex !== -1 && endIndex !== -1 && endIndex > startIndex) {
    ...
    next = `${existing.slice(0, startIndex)}${block}${existing.slice(afterEnd).replace(/^\r?\n/, '')}`;
    mode = 'updated';
  } else if (isStandaloneNadoAgentContext(existing)) {
    next = block;
    mode = 'migrated';
  } else if (existing.trim()) {
    next = `${existing.replace(/\s*$/u, '')}\n\n${block}`;
  } else {
    next = block;
  }
  ...
  return { file: target, mode, marker };
}
